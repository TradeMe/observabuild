...
export class DoTask implements ITaskAction {
    ...
    constructor(private _task: IDoTask, private _observer: Subscriber<TaskEvent>) {
        this._observer.next(new TaskStart(this._task, this._startTime));
    }

    static create(task: IDoTask): Observable<TaskEvent> {
        return new Observable<TaskEvent>((observer) => {
            let action = new DoTask(task, observer);
    ...
    copyFolder(srcPath: string, destPath: string | undefined): void {
        ...
        mkdirp.sync(destPath); // ensure destination path exists
        let ncpOptions = {
            stopOnErr: true,
            limit: 16
        };
